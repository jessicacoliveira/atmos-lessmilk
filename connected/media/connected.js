...
Game.Menu.prototype = {
  create: function () {
    ...
    game.add
      .text(w / 2, 50, "connected", { font: "30px Arial", fill: "#2c3e50" })
      .anchor.setTo(0.5, 0.5);
    var label_start = game.add.text(
      w / 2,
      h - 50,
      "connect the two dots to start",
      { font: "20px Arial", fill: "#2c3e50" },
    );
  ...
  update: function () {
    var bool1 = Phaser.Rectangle.contains(
      this.dot1.body,
      game.input.activePointer.x,
      game.input.activePointer.y,
    );
    var bool2 = Phaser.Rectangle.contains(
      this.dot2.body,
      game.input.activePointer.x,
      game.input.activePointer.y,
    );
...
Game.Play.prototype = {
  ...
  build_world: function () {
    for (var i = 0; i < 6; i++)
      for (var j = 0; j < 6; j++) this.add_dot(i, j, true);
  },
  add_dot: function (i, j, type) {
    ...
    if (type)
      game.add
        ...
        .to({ x: 1, y: 1 }, 400)
        .start();
    else game.add.tween(dot.scale).delay(400).to({ x: 1, y: 1 }, 400).start();
  },
  try_select_a_dot: function (e) {
    this.dots.forEachAlive(function (e) {
      var bool = Phaser.Rectangle.contains(
        e.body,
        game.input.activePointer.x,
        game.input.activePointer.y,
      );
      if (e.selected || !bool) return;
      if (this.type == -1) this.type = e.frame;
      if (this.type == e.frame && this.in_range(e)) this.select_a_dot(e);
      else this.unselect_all_dots();
    }, this);
  },
  select_a_dot: function (e) {
    if (sound) {
      if (this.progress.width / 28 <= 3) this.tap1_s.play("", 0, 0.5, false);
      else if (this.progress.width / 28 <= 7)
        this.tap2_s.play("", 0, 0.7, false);
      else this.tap3_s.play("", 0, 0.9, false);
    }
    e.selected = true;
    e.frame += 1;
    this.count += 1;
  ...
  move_dots_down: function () {
    ...
    this.dots.forEachAlive(function (e) {
      if (e.move_y == 0) return;
      var coef = e.move_y / tile_size;
      e.pos_i += coef;
      game.add
        .tween(e)
        .delay(100)
        .to({ y: e.y + e.move_y }, 100 * coef)
        .start();
      e.move_y = 0;
    }, this);
    this.deleted_dots = [];
  ...
  add_missing_dots: function () {
    var min = [6, 6, 6, 6, 6, 6];
    this.dots.forEachAlive(function (e) {
      if (e.pos_i < min[e.pos_j] && e.alive2) min[e.pos_j] = e.pos_i;
    }, this);
    for (var i = 0; i < 6; i++)
      for (var j = 0; j < min[i]; j++) this.add_dot(j, i, false);
  },
  ...
  create: function () {
    ...
    game.add
      .text(
        w / 2,
        h / 2,
        "you ended your 20 turns\n\nyou scored: " +
          score +
          "\nyour best score: " +
          best_score,
        { font: "30px Arial", fill: "#2c3e50", align: "center" },
      )
      .anchor.setTo(0.5, 0.5);
    if (score > best_score) {
      best_score = score;
    ...
    emitter.start(false, 5e3, 200);
  },
  update: function () {
    if (game.input.activePointer.isDown) this.game.state.start("Play");
  },
};
